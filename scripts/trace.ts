...
for (let t=0;t<40;t++){
  ...
  const pick = legal.map(w=>({w,tailCount:WORDS.filter(x=>x.word[0]===getLastKey(w.word)&&!used.has(x.word)).length,len:w.word.length}))
    .sort((a,b)=> b.tailCount-a.tailCount || b.len-a.len)[0].w;
  const atk = applyPlayerAttack(state, pick); state=atk.state; used.add(pick.word); lastChar=getLastKey(pick.word);
